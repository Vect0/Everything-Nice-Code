...
$row1 = mysqli_fetch_assoc($sql07);
    ...
    while($row = mysqli_fetch_assoc($sql06)) {
        ...
        " - Total: " . $row["BoughtTotal"] . " - Date ordered: " . $row["order_date"] .
         ...
         $sql11 = " SELECT * FROM sales WHERE Sell_Date BETWEEN '$from_date' AND '$to_date'";

         $sql12 = "SELECT SUM(total) AS sum1 FROM sales WHERE Sell_Date BETWEEN '$from_date' AND '$to_date' ";
...
echo "You purchased a total of $".  $summ . " worth of inventory between " . $from_date . " and " . $to_date . ".";
    ...
    echo "You purchased a total of $" . $summ . " and sold a total of $" . $summ1 . " worth of inventory between " . $from_date . " and " . $to_date . ".  <br><br>";
} else echo " ";
...
if(($summ - $summ1) >= 0){
  if(mysqli_num_rows($sql06)>0 && (mysqli_num_rows($outcome) >0)) echo "Your profit between " . $from_date . " and " . $to_date . " is $" . ($summ - $summ1) . ". ";
} else if(($summ - $summ1) < 0) {
  if(mysqli_num_rows($sql06)>0 && (mysqli_num_rows($outcome) >0)) echo "Your loss between  " . $from_date . " and " . $to_date . " is $" . (($summ - $summ1)*-1) . ". ";
}
...
<br><br><br><br>
<div><button class="submit" type="button" onclick="window.location.href='ViewDatesProfitPrompt.php'"> Back </div>
...
<div class="popup" onclick="myFunction()">Help
  <span class="popuptext" id="myPopup">
    		...
    		Displayed is the information regarding your date search.  The profits or losses that were earned between the dates you entered will appear here.  
                    <br><br>
